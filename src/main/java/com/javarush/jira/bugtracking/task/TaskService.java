...
public class TaskService {
    ...
    public Duration taskInProgress(TaskToExt task){
        if(task.getStatusCode().equals("in_progress") || task.getStatusCode().equals("todo") ){
            throw new AppException("The task is still in the in_progress or todo status");
        }
        ...
        LocalDateTime readyForReview = LocalDateTime.now();
        for(var act : activities){
            if(act.getStatusCode().equals("in_progress")){
                inProgress = act.getUpdated();
            } else if (act.getStatusCode().equals("ready_for_review")) {
                readyForReview = act.getUpdated();
            }
        }
        return Duration.between(readyForReview,inProgress);
    }

    public Duration taskInTest(TaskToExt task){
        if(!task.getStatusCode().equals("done")){
            throw new AppException("The task is not yet completed");
        }
        List<Activity> activities = activityHandler.getRepository().findAllByTaskIdOrderByUpdatedDesc(task.getId());
        LocalDateTime review = LocalDateTime.now();
        LocalDateTime done = LocalDateTime.now();
        for(var act : activities){
            if(act.getStatusCode().equals("ready_for_review")){
                review = act.getUpdated();
            } else if (act.getStatusCode().equals("done")) {
                done = act.getUpdated();
            }
        }
        return Duration.between(done,review);
    }
}
